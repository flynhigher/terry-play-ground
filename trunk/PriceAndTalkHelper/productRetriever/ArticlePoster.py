...
class ArticlePoster_Exception(Exception): pass
...
class ArticlePoster:
    ...
    def __init__(self, user_id):
        ...
        self.nowtime_str = now.strftime(self.__tformat)
        self.out_filename = 'output' + self.nowtime_str + '.xml'
#        if config['defines'].has_key('outputpath'):
#            self.out_file = config['defines']['outputpath'] + self.out_filename
#        else:
        self.out_file = self.out_filename
        self.doc_open()
        self.xml_articles = self.xml.xpathEval('/posts')[0]
        self.xml.setProp('pubDate', self.nowtime_str)
    ...
    def describe(self, parent, description):
        xml_d = parent.xpathEval('content')
        if len(xml_d) is 0:
            xml_d = parent.newChild(None, 'content', description)
        else:
    ...
    def article_add(self, article):
      xml_article = self.xml_articles.newChild(None, 'post', None)
      self.writeElement(xml_article, 'category', 'Amazon');
      self.writeElement(xml_article, 'regdate', self.nowtime_str);
      self.writeElement(xml_article, 'user_id', self.user_id);
      self.writeElement(xml_article, 'nick_name', self.user_id);
#      extra_vars = xml_article.newChild(None, 'extra_vars', None)
#      self.writeElement(extra_vars, 'source', entry_info['link'])
      self.writeElement(xml_article, 'title', article.subject)
      self.writeElement(xml_article, 'content', article.body)
      return True
    ...
    def __upload(self):
        ...
        url = 'http://www.priceandtalk.com/etc/upload.php'
    ...
    def __post(self):
        ...
        url = 'http://www.priceandtalk.com/index.php'
