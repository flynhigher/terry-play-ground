...
class Article_Poster:
		...
		def __init__(self, board_id=5, working_folder=None, config=None):
#			self.server_url = 'http://www.priceandtalk.com'
			...
			if config and config['defines'].has_key('outputpath'):
				...
				self.out_file = os.path.join(working_folder if working_folder is not None else "", self.out_filename)
...
#ucprice.com
						...
						2: ('84', 'Hot Deal'),
						3: ('106', 'RSS'),
						4: ('1365254', 'Amazon Korean Products'),
#priceandtalk.com
#						1: ('109', 'Shopping List'),
#						2: ('93', 'Hot Deal'),
#						3: ('128', 'RSS'),
...
#        f.storlines('STOR ' + self.server_out_filepath + self.out_filename, open(self.out_file,'r'))
		...
		def __post(self):
				...
				headersxml = {'Content-type': 'application/xml', 'Content-Length': str(len(login))}
