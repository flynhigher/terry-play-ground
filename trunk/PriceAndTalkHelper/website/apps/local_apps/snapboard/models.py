...
__all__ = [
    ...
    'Board', 'Invitation', 'Group', 'Post', 'Moderator',
...
class PermissionError(PermissionDenied):
    ...
    permissions defined by Board objects.
    '''
    pass
...
class Board(models.Model):
    ...
    view_perms = models.PositiveSmallIntegerField(_('view permission'), 
        ...
        help_text=_('Limits the board\'s visibility.'))
    ...
    post_perms = models.PositiveSmallIntegerField(_('post permission'),
        ...
        'post in the board.'), default=USERS)
    new_post_perms = models.PositiveSmallIntegerField(
        _('create post permission'), choices=PERM_CHOICES_RESTRICTED, 
        help_text=_('Limits the ability to create new posts in the '\
        'board. Only users with permission to post can create new posts,'\
        'unless a group is specified.'), default=USERS)

    view_group = models.ForeignKey(Group, verbose_name=_('view group'),
        blank=True, null=True, related_name='can_view_board_set')
    read_group = models.ForeignKey(Group, verbose_name=_('read group'),
        blank=True, null=True, related_name='can_read_board_set')
    post_group = models.ForeignKey(Group, verbose_name=_('post group'),
        blank=True, null=True, related_name='can_post_board_set')
    new_post_group = models.ForeignKey(Group, verbose_name=_('create post group'),
        blank=True, null=True, related_name='can_create_post_board_set')

    objects = managers.BoardManager()    # adds board_count

    def __unicode__(self):
        return self.label

    def moderators(self):
        mods = Moderator.objects.filter(board=self.id)
        if mods.count() > 0:
    ...
    def can_create_post(self, user):
        flag = False
        if self.new_post_perms == USERS:
            flag = user.is_authenticated()
        elif self.new_post_perms == CUSTOM:
            flag = user.is_superuser or (user.is_authenticated() and self.new_post_group.has_user(user))
        return flag
...
class Post(models.Model):
    ...
    # Board sticky - will show up at the top of board listings.
    csticky = models.BooleanField(default=False, verbose_name=_('board sticky'))
    ...
    def can_edit(self, user):
        flag = False
        flag = user.is_superuser or (user.is_authenticated() and self.user.id == user.id)
        return flag
    ...
    def notify(self, **kwargs):
        ...
        if not self.previous:
            ...
            recipients = set((wl.user for wl in WatchList.objects.filter(board=self.board) if wl.user not in all_recipients))
            if recipients:
                notification.send(
                    recipients,
                    'new_post_in_watched_board',
                    {'post': self}
                )
                all_recipients = all_recipients.union(recipients)

    def get_absolute_url(self):
        return reverse('snapboard_post', args=(self.id,))

    def get_edit_form(self):
        from forms import PostForm
        return PostForm(initial={'post':self.text, 'subject':self.subject, 'private':self.private})
#        return PostForm(instance=self)
...
class AbuseReport(models.Model):
    ...
    set to disallow further abuse reports on said board.
    '''
...
class WatchList(models.Model):
    """
    Keep track of who is watching what board.  Notify on change (sidebar).
    """
    user = models.ForeignKey(User, verbose_name=_('user'), related_name='sb_watchlist')
    post = models.ForeignKey(Board, verbose_name=_('post'))
...
class UserSettings(models.Model):
    ...
    tpp = models.IntegerField(
            choices = ((5, '5'), (10, '10'), (20, '20'), (50, '50')),
            default = 20,
            help_text = _('Board per page'), verbose_name=_('boards per page'))
...
#            help_text = "Email notifications for watched discussions.", verbose_name=_('notify'))
    ...
    frontpage_filters = models.ManyToManyField(Board,
            null=True, blank=True,
            help_text = _('Filter the list of all topics on these boards.'), verbose_name=_('front page boards'))
