...
def rpc_csticky(request, **kwargs):
    ...
    if _toggle_boolean_field(kwargs['post'], 'csticky'):
        return {'link':_('unset csticky'), 'msg':_('This post is sticky in its board.')}
    else:
        return {'link':_('set csticky'), 'msg':_('Removed post from board sticky list')}


def rpc_gsticky(request, **kwargs):
    if not request.user.is_staff:
        raise PermissionDenied
    if _toggle_boolean_field(kwargs['post'], 'gsticky'):
        return {'link':_('unset gsticky'), 'msg':_('This post is now globally sticky.')}
    else:
        return {'link':_('set gsticky'), 'msg':_('Removed post from global sticky list')}


def rpc_close(request, **kwargs):
    if not request.user.is_staff:
        raise PermissionDenied
    if _toggle_boolean_field(kwargs['post'], 'closed'):
        return {'link':_('open post'), 'msg':_('This discussion is now CLOSED.')}
    else:
        return {'link':_('close post'), 'msg':_('This discussion is now OPEN.')}


def rpc_watch(request, **kwargs):
    thr = kwargs['post']
    if not thr.board.can_read(request.user):
        raise PermissionError
    try:
        ...
        wl.delete()
        return {'link':_('watch'),
                'msg':_('This post has been removed from your favorites.')}
    except WatchList.DoesNotExist:
        # create it
        wl = WatchList(user=request.user, post=thr)
        wl.save()
        return {'link':_('dont watch'),
                'msg':_('This post has been added to your favorites.')}
