...
#
# modified Oct. 7, 2009 based on Amazon's requirement
	url = "https://" + __supportedLocales[getLocale()] + "/onca/xml?"
	argv['Service'] = 'AWSECommerceService' #add Service to url param to argv so it can be sorted
...
class pagedIterator:
	...
	def next(self):
		if self.__index < self.__len and self.__index >= 0:
			self.__index = self.__index + 1
			try:
				return self.__getitem__(self.__index-1)
			except IndexError, e:
				print ("__index: ", self.__index - 1)
				print e
				raise StopIteration
		else:
			raise StopIteration
...
def XMLCartCreate(Items, Quantities, ResponseGroup=None, AWSAccessKeyId=None):
	...
	__fromListToItems(argv, Items, 'OfferListingId', Quantities)
	return query(buildRequest(argv))
